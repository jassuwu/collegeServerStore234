...
DECLARE 
   ...
   c_gender person.gender%type; 
   CURSOR c_person is 
      SELECT name, age, gender FROM person; 
BEGIN 
   OPEN c_person; 
   LOOP 
   FETCH c_person into c_name, c_age, c_gender; 
      EXIT WHEN c_person%notfound; 
      dbms_output.put_line(c_name || ' is ' || c_age || ' y/o and is a ' || c_gender || '.'); 
   END LOOP; 
   CLOSE c_person; 
END; 
/
...
DECLARE
  c_name person.name%type;
  c_count int;
  cursor c_person is SELECT pizzeria, count(*) from Frequents GROUP BY pizzeria ORDER BY count(*) DESC;
BEGIN
  OPEN c_person;
  LOOP
  FETCH c_person into c_name, c_count;
  EXIT WHEN c_person%notfound;
  dbms_output.put_line(c_name || ' has ' || c_count || ' frequents.');
  END LOOP;
  CLOSE c_person;
END;
/
